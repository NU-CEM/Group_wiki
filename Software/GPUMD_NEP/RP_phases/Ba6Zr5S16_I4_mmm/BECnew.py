...
for i in atoms:
    for j in coordinates:
         for k in wyckoff:
            print(i,j,k)
            os.system("python3 BEC.py --name %s -p %s --kx 16 16 16 --ky 16 16 16 --kz 16 16 16 -c %s >> output_%s_%s_%s"%(i,k,j,i,j,k))
            os.system('pwd')
            os.system("mv %s_disp_0.0 %s_disp_0.0_%s_%s"%(i,i,j,k))
            os.system("mv %s_disp_0.0025 %s_disp_0.0025_%s_%s"%(i,i,j,k))
