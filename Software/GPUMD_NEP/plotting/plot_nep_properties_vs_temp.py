# this script assumes that the thermo.out file is in the same directory
...
window_average = 100
df['potential_energy'] = df['potential_energy']/20000 # divide by number of atoms
df['potential_energy'] = df['potential_energy'] + 1.81440132003 # !!!! this is the perovskite Pnma energy from the NEP at 0K, please change this if you use it for any other system !!!! this is done to have the 0K energy set to 0 meV
df['potential_energy'] = df['potential_energy'] - (1.5*k_B*df['temperature']) # removing 1/2k_BT from the energy  
...
ax = axes
# plotting all lattice constants as "equivalent"
